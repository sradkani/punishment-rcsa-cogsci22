...
def prospect_theory_value(x, power):
    if x>0:
        return x**power
    else:
        return -1 * ((-x)**power)
...
def get_available_actions(key):
    if key=='Punisher':
        available_actions = [i for i in np.arange(10,101,10).tolist()]
    return available_actions

def get_selfish_utilities(key, available_actions):
    if key=='Punisher':
        selfish_utilities = {i: -(i/100)*45 for i in np.arange(10,101,10).tolist()}   # how many percent of the endownment they spent on punishment * 50
    return selfish_utilities

def get_social_utilities(key, available_actions):
    if key=='Punisher':
        social_utilities = {i: (0 if i==0 else sigmoid(i/100, 10, 0.5)*10) for i in np.arange(10,101,10).tolist()}
    return social_utilities

def get_target_utilities(key, available_actions):
    if key=='Punisher':
        target_utilities = {i: (-i/100)*100 for i in np.arange(10,101,10).tolist()}
